...
void main() {
  ...
  test('importBackupV2 remaps account references and transfer links', () async {
    final payload = {
      ...
      'dbSchemaVersion': 4,
      'settings': {'currency': 'USD', 'themeMode': 'dark'},
  ...
  test(
    'importBackupV2 complex fixture payload with transfers, budgets, mixed events, and balance reconciliation',
    () async {
      ...
      final checking = dbAccounts.firstWhere(
        (a) => a.name == 'Checking Wallet',
      );
      final savings = dbAccounts.firstWhere(
      ...
      final oneOff = dbTasks.firstWhere(
        (t) => t.title == 'One-off Task Completed',
      );
      expect(oneOff.isCompleted, isTrue);
      expect(oneOff.recurrencePattern, isNull);

      final recurring = dbTasks.firstWhere(
        (t) => t.title == 'Daily Exercise Recurring',
      );
  ...
  test(
    'importBackupV2 rolls back fully on unresolved account mapping',
    () async {
      await db.saveTask(
      ...
      expect(
        afterTasks.length,
        1,
        reason: 'transaction should rollback all changes',
      );
    },
  );
}
